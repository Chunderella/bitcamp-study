...
public class ServerAppbackup3 {
  public static void main(String[] args) { 

    //servletMap 에서 사용할 로컬 변수를 미리 준비한다.(로컬클래스가 정의되기 전에)
    ...
    //스레드로 만드는 대신에 스레드가 실행할 수 있는 클래스로 변경한다.
    class RequestRunnable implements Runnable { 
      ...
      public void run() {
        try (Socket socket = this.socket;
          ...
          //로컬 클래스는 바깥메서드의 로컬 변수를 자신의 멤버인 것 처럼 사용할 수 있다.
