...
public class Exam01 {
  ...
  public static void main(String[] args) {
    //sedan과 truck 두클래스의 공통점을 추출하여 수퍼클래스를 정의한 후에
    //    두클래스의 사용법은 크게 바뀌는 것이 아니다.
    //    단지 유지보수하기 쉬워질뿐이다.
    Sedan s = new Sedan();
    s.run();
    ...
    //이렇게 car 클래스가 존재하면,
    //어떤 개발자가 다음과 같이 이 car 클래스를 사용하려는 경우가 있을 수 있다.
    //문제는 car 클래스의 목적이 소스코드를 보다 쉽게 관리하기 위해 만든 클래스이지
    //직접 사용하려고 만든 클래스가 아니다.
    //그럼에도 불구하고 다른 개발자가 다음과 같이 car 클래스를 쓴다면
    //이를 막을 수 없다.
